...
export const schema = Schema.object({
  apiKey: Schema.string().description('apexstatus 网页 API').role('secret').required(),
})
...
export function apply(ctx: Context, config: Config) {
  ...
  .action(
    async ({ options }, username) => {
      ...
      var bp: string
      var says: string
      if (prvBattlePass == -1) {bp = '上个赛季的通行证是一点没打'}
      else if (prvBattlePass == 111) {bp = '上个赛季的通行证是打的满满当当'}
      else bp = '上个赛季的通行证等级是: '+prvBattlePass
      if (rp <= 1000) {says = '嗯?菜鸟段?还是...不打排位?'}
      else if (rp > 1000 && rp <= 3000) {says = '青铜局?才刚刚开始~'}
      else if (rp > 3000 && rp <= 5400) {says = '到白银了,好起来了~'}
      else if (rp > 5400 && rp <= 8200) {says = '黄金!开始乱斗!'}
      else if (rp > 8200 && rp <= 11400) {says = '白金!进入上三段!'}
      else if (rp > 11400 && rp <= 15000) {says = '钻石!上钻咯兄弟们!'}
      else if (rp < 15000) {says = '我超!大师!猎杀!'}
      const disp = '当当当!\n你查询的用户叫: '+ target +'\n他的等级是: '+ playerLV + '\n' + bp +'\n本赛季的排位赛点数为:'+rp + '\n' + says
